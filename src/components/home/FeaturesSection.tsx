          ...
          {features.map((feature, index) => {
            ...
            return (
              <Link
                key={index}
                to={feature.link}
                className="group relative glass-card rounded-2xl p-6 md:p-8 hover:border-primary/30 transition-all duration-500 animate-fade-in text-left"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                {/* Icon */}
                <div
                  className={`relative w-14 h-14 rounded-xl bg-gradient-to-br ${feature.gradient} flex items-center justify-center mb-6 shadow-lg group-hover:scale-110 transition-transform duration-300`}
                >
